import pandas as pd, numpy as np, point_utils

def get_matches_from_list(filenames,sheetnames,column_keys,skiprows,shift=0,stage=''):
    matches=[]
            ...
            for i in df.T.iteritems():
                ...
                for j in column_keys:
                    if type(column_keys[j])==dict:
                        ...
                        match[j]=filename+'#'+sheetname+'$'+stage+'%'+str(i[1][column_keys[j]+shift])
                    else:
                        match[j]=i[1][column_keys[j]+shift]
                matches.append(match)
    return matches

def get_matches_from_tournament(filenames,sheetnames,skiprows=0,shift=0,drops=[],stage=''):
    matches=[]
    if type(filenames)!=list:
        filenames=[filenames]
    if type(sheetnames)!=list:
        ...
        for sheetname in sheetnames:
            ...
            for stg in range(1,n):
                ...
                for j in range(len(names)//2):
                    ...
                    match={'match_type':sheetname+'$'+stage,'aka':{'name':names[j*2],
                        'point1':aka},
...
def get_matches_from_table(filename,sheetnames,skiprows,nrows=0,shift=0,drops=[],stage='',point_shift=1):
    matches=[]
    if type(sheetnames)!=list:
        sheetnames=[sheetnames]
    for sheetname in sheetnames:
        ...
        for i in range(len(df.index)//2):
            for j in range(1,len(df.index)//2+1):
                if i<(j-1):
                    match={'match_type':sheetname+'$'+stage,'aka':{'name':df.loc[i*2][0],
                              'point1':df.loc[i*2+point_shift][j*2],'point2':df.loc[i*2+1][j*2+1]},
                     'shiro':{'name':df.loc[(j-1)*2][0],
                              'point1':df.loc[(j-1)*2+point_shift][(i+1)*2],'point2':df.loc[(j-1)*2+1][(i+1)*2+1]}}
                    matches.append(match)    
    return matches

def get_matches_from_table_oneliner(filename,sheetnames,skiprows,nrows=0,shift=0,point_shift=1,drops=[],stage='',rowheight=1):
    matches=[]
    if rowheight>1:
...
def get_matches(year,competition,stage=''):
    ...
    if year==2018:
        ...
        if competition=='CN':
            ...
            for i in range(len(sheetnames)):
                ...
                matches=matches+get_matches_from_table_oneliner(filename,sheetname,skiprow,
                                        shift=shift,point_shift=point_shift,nrows=nrows,stage='finals')
            #part 2
            ...
            #part3
            sheetname=['IM_manual']
            matches=matches+get_matches_from_tournament(filename,sheetname,stage='finals')
        elif competition=='CR':
            ...
            for i in range(len(sheetnames)):
                ...
                nrows=nrowss[i]
                matches=matches+get_matches_from_table_oneliner(filename,sheetname,skiprow,
                                            point_shift=point_shift,shift=shift,nrows=nrows,stage='finals')
            #part3
            sheetname=['IM_manual']
            matches=matches+get_matches_from_tournament(filename,sheetname,stage='finals')
    elif year==1999:
        if competition=='CR':
            filename=path+'Results_CR1999.xlsx'
            #part 1
            sheetnames=[['IF'],['IF'],['IC']]
            stages=['finals','finals','finals']
            skiprows=[8,18,6]
            shifts=[1,1,0]
            for i in range(len(sheetnames)):
                sheetname=sheetnames[i]
                ...
                stage=stages[i]
                shift=shifts[i]
                matches=matches+get_matches_from_table(filename,sheetname,skiprow,nrows=8,shift=shift,stage=stage)
            #part2
            sheetname=['IM_manual']
            matches=matches+get_matches_from_tournament(filename,sheetname,stage='finals')
            #part3
        ...
        if competition=='CR':
            filename=path+'1998.10.24-25 - CR - Results.xlsx'
            #part 1
            sheetnames=[['IC_meciuri'],['IJ_meciuri'],['IM_schema']]
            stages=['finals','finals','']
            skiprows=[6,5,11]
            shifts=[0,0,1]
            cks=[column_keys_012_653_789,column_keys_012_653_789,column_keys_012_543]
            for i in range(len(sheetnames)):
                sheetname=sheetnames[i]
                skiprow=skiprows[i]
                stage=stages[i]
                ...
                matches=matches+get_matches_from_list(filename,sheetname,ck,skiprow,shift=shift,stage=stage)
            #part2
            sheetname=['IM_schema_manual']
            matches=matches+get_matches_from_tournament(filename,sheetname,stage='finals')
    elif year==1997:
        if competition=='CR':
            filename=path+'Results.xlsx'
            sheetnames=[['IF']]
            shifts=[1]
            point_shifts=[0]
            skiprows=[4]
            ...
            for i in range(len(sheetnames)):
                ...
                stage=stages[i]
                point_shift=point_shifts[i]
                matches=matches+get_matches_from_table(filename,sheetname,skiprow,
        ...
        if competition=='CR':
            ...
            shifts=[0,-1,6]
            skiprows=[7,20,20]
            stages=['pool','finals','finals']
            cks=[column_keys_012_543,column_keys_312_543,column_keys_312_543]
            for i in range(len(sheetnames)):
                sheetname=sheetnames[i]
                skiprow=skiprows[i]
                shift=shifts[i]
            ...
            for i in range(len(sheetnames)):
                ...
                matches=matches+get_matches_from_table_oneliner(filename,sheetname,skiprow,rowheight=rowheight,
