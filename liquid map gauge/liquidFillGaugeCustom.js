...
function loadLiquidFillGauge(elementId, shapeID, value, config) {
    var BBox = d3.select('#' + shapeID).node().getBBox()

    function liquidFillGaugeDefaultShape(g, radius, fillCircleRadius, color) {
        g.append("polygon")
            .attr("points", parseFloat(radius - fillCircleRadius) + ',' + parseFloat(radius - fillCircleRadius) + ' ' +
                parseFloat(radius) + ',' + parseFloat(radius + fillCircleRadius) + ' ' +
                parseFloat(radius + fillCircleRadius) + ',' + parseFloat(radius + fillCircleRadius))
            .style("fill", color)
    ...
    var data = [];
    var Nsegments = 40
    for (var i = -Nsegments * waveClipCount; i <= 2 * Nsegments * waveClipCount; i++) {
        data.push({ x: i / (Nsegments * waveClipCount), y: (i / (Nsegments)) });
    }
    ...
    var fillCircleGroup = gaugeGroup.append("g")
        .attr("clip-path", "url(#clipWave" + shapeID + ")");

    fillCircleGroup.append(function () {
        var shape = d3.select('#' + shapeID);
        shape.attr('transform', 'translate(' + (-locationX) + ',' + (-locationY) + ')')
